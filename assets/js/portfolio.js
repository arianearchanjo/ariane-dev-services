  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        setTimeout(() => { t.style.width = (t.getAttribute('data-w') || '0') + '%'; }, 200);
...
/* ──────────────────────────────────────────────
   7. TYPED TEXT — frases comerciais
   ────────────────────────────────────────────── */
(function initTyped() {
  ...
  const phrases = [
    ...
    'Código limpo. Entrega no prazo.',
    'Do briefing ao ar em dias.',
  ];
  ...
  function type() {
    ...
    if (!isDeleting) {
      ...
      if (charIndex === current.length) {
        isPausing = true;
        setTimeout(() => { isPausing = false; isDeleting = true; setTimeout(type, 60); }, 2200);
        return;
      }
      setTimeout(type, 65);
    } else {
